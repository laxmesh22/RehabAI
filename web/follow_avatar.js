...
const std = (color, roughness = 0.85) => new THREE.MeshStandardMaterial({ color, roughness, metalness: 0 })
...
const sideSign = (s) => (s === 'left' ? 1 : -1)     // model faces +z, so its left arm is at +x
...
class GoniometerArc {
  ...
  setTarget(deg) { this.place(this.tick, deg) }

  setLive(deg) {
    this.live.visible = deg != null && Number.isFinite(deg)
    if (this.live.visible) this.place(this.live, deg)
  }
}
